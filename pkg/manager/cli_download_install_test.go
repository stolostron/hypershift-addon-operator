...
func (suite *CLIDownloadTestSuite) TestEnableHypershiftCLIDownloadXCSVLookupError() {
	...
	suite.Require().NoError(
		o.Client.List(context.Background(), csvList),
		"must list CSVs to clear fixtures before exercising the CSV-lookup-failure path",
	)
	for i := range csvList.Items {
		suite.Require().NoError(
			o.Client.Delete(context.Background(), &csvList.Items[i]),
			"must delete leftover CSV so no MCE CSV exists for this test",
		)
	}
	...
	suite.True(
		...
		"error should wrap context.DeadlineExceeded via %%w so callers can check it with errors.Is",
	)
}
...
func (suite *CLIDownloadTestSuite) TestEnableHypershiftCLIDownloadXSkipsWithoutImage() {
	...
	csvList := &operatorsv1alpha1.ClusterServiceVersionList{}
	suite.Require().NoError(
		o.Client.List(ctx, csvList),
		"must list CSVs to clear fixtures before leaving only the upstream CSV in place",
	)
	for i := range csvList.Items {
		suite.Require().NoError(
			o.Client.Delete(ctx, &csvList.Items[i]),
			"must delete leftover CSV so only the upstream CSV created below is present",
		)
	}
	...
	suite.Require().NoError(
		err,
		"EnableHypershiftCLIDownload should skip enabling the hcp CLI download without failing when no image is available",
	)
...
func (suite *CLIDownloadTestSuite) TestEnableHypershiftCLIDownloadXDeployError() {
	...
	suite.Require().NoError(
		o.Client.Get(ctx, addonDeploymentNN, addonDeployment),
		"hypershift-addon-manager deployment should exist from earlier tests",
	)
	...
	suite.Require().Error(
		err,
		"EnableHypershiftCLIDownload should fail when the addon manager deployment used for the owner reference is missing",
	)
	suite.Contains(
		err.Error(),
		"deploy HypershiftCLIDownload",
		"error should be wrapped with context about the failing operation",
	)
